 ...
 * 
 * This is Gamification Game service to manage top level game in Gamification
 * Framework
...
@Api(value = "/gamification/games", authorizations = {
		@Authorization(value = "game_auth", scopes = {
...
@SwaggerDefinition(info = @Info(title = "Gamification Game Service", version = "0.1", description = "Gamification Game Service for Gamification Framework", termsOfService = "http://your-terms-of-service-url.com", contact = @Contact(name = "Muhammad Abduh Arifin", url = "dbis.rwth-aachen.de", email = "arifin@dbis.rwth-aachen.de"), license = @License(name = "your software license name", url = "http://your-software-license-url.com")))
@ManualDeployment
...
public class GamificationGameService extends RESTService {
	...
	/**
	 * Function to delete directories of an game in badge service and point service
	 * file system
	 * 
	...
	private boolean cleanStorage(String gameId) throws AgentNotFoundException, InternalServiceException,
		...
		try {
			result = Context.getCurrent().invoke(
					"i5.las2peer.services.gamificationBadgeService.GamificationBadgeService@0.1", "cleanStorageRMI",
					new Serializable[] { gameId });
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (result != null) {
			Context.getCurrent().monitorEvent(this, MonitoringEvent.RMI_SENT,
					"Clean Badge Service Storage : " + gameId + result);

			if (((Integer) result) == 1) {
				Context.getCurrent().monitorEvent(this, MonitoringEvent.RMI_SUCCESSFUL,
						"Clean Badge Service Storage : " + gameId);
				Object res = null;
				try {
					res = Context.getCurrent().invoke(
							"i5.las2peer.services.gamificationPointService.GamificationPointService@0.1",
							"cleanStorageRMI", new Serializable[] { gameId });
				} catch (Exception e) {
					e.printStackTrace();
				}
				if (res != null) {
					Context.getCurrent().monitorEvent(this, MonitoringEvent.RMI_SENT,
							"Clean Point Service Storage : " + gameId + res);
					if (((Integer) res) == 1) {
						Context.getCurrent().monitorEvent(this, MonitoringEvent.RMI_SUCCESSFUL,
								"Clean Point Service Storage : " + gameId);
		...
		Context.getCurrent().monitorEvent(this, MonitoringEvent.RMI_FAILED,
				"Clean Badge or Point Service Storage : " + gameId);
		...
		try {
			...
			if (!isGameIdValid(gameid)) {
				objResponse.put("message",
						"Invalid game ID. Game ID MUST NOT be blank and MUST NOT contain upper case characters. Max length is 20.");
			...
			try {
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_1, "" + randomLong,
						true);
				gameAccess.addNewGame(conn, newGame);
				gameAccess.addMemberToGame(conn, newGame.getId(), name);
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_2, "" + randomLong,
						true);
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_3, "" + name, true);
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_4, "" + newGame.getId(),
						true);
				objResponse.put("message", "New game created");
				return Response.status(HttpURLConnection.HTTP_CREATED).entity(objResponse.toJSONString())
						.type(MediaType.APPLICATION_JSON).build();
	...
	private boolean isGameIdValid(String gameId) {
		 ...
		 * implement this correctly. For example, a function might try to query a
		 * concrete game
		 * using a schema name:
		 * SELECT * FROM manager.game_info WHERE game_id = schema_name
		 * This query will fail if the game ID is 'testGameId' because the schema name
		 * will be 'testgameid'.
		 *
		 * Instead of fixing this in the SQL code, which is very hard to maintain, we
		 * will enforce this
	...
	@Path("/data/{gameId}")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "getGameDetails", notes = "Get an game data with specific ID")
	@ApiResponses(value = {
	...
	public Response getGameDetails(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"GET " + "gamification/games/data/" + gameId, true);
	...
	@Path("/data/{gameId}")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Delete Game", notes = "This method deletes an Game")
	@ApiResponses(value = {
	...
	public Response deleteGame(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"DELETE " + "gamification/games/data/" + gameId, true);
		...
		try {
			...
			try {
				if (cleanStorage(gameId)) {
					...
					if (gameAccess.deleteGameDB(conn, gameId)) {
						...
						Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_13, "" + name,
								true);
			...
			} catch (AgentNotFoundException | InternalServiceException | InterruptedException | TimeoutException e) {
				e.printStackTrace();
	...
	/**
	 * Get all game list separated into two categories. All games registered for the
	 * member and other games.
	 * 
	 * 
	 * @return HTTP Response with the returnString
	 */
	...
	@Path("/list/separated")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "getSeparateGameInfo", notes = "Get all game list separated into two categories. All games registered for the member and other games.")
	@ApiResponses(value = {
	...
	public Response getSeparateGameInfo() {
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"GET " + "gamification/games/list/separated", true);
		...
		try {
			conn = dbm.getConnection();
			// List<List<GameModel>> allGames =
			// gameAccess.getSeparateGamesWithMemberId(conn,name);
			...
			return Response.status(HttpURLConnection.HTTP_OK).entity(allGames.toJSONString())
					.type(MediaType.APPLICATION_JSON).build();
	...
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "removeMemberFromGame", notes = "delete a member from an game")
	@ApiResponses(value = {
	...
	public Response removeMemberFromGame(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"DELETE " + "gamification/games/data/" + gameId + "/" + memberId, true);
		...
		try {
			...
			e.printStackTrace();
			objResponse.put("message",
					"Cannot remove member from Game. Error checking game ID exist " + e.getMessage());
			Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR, (String) objResponse.get("message"));
			return Response.status(HttpURLConnection.HTTP_BAD_REQUEST).entity(objResponse.toJSONString())
					.type(MediaType.APPLICATION_JSON).build();
		}
	...
	@ApiOperation(value = "addMemberToGame", notes = "add a member to an game")
	public Response addMemberToGame(
			@ApiParam(value = "Game ID", required = true) @PathParam("gameId") String gameId,
			@ApiParam(value = "Member ID", required = true) @PathParam("memberId") String memberId) {
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"POST " + "gamification/games/data/" + gameId + "/" + memberId, true);
		...
		try {
			...
			try {
				// problem here, player is re-added, which i dont think makes sense
				if (!gameAccess.isMemberRegisteredInGame(conn, memberId, gameId)) {
					gameAccess.addMemberToGame(conn, gameId, memberId);
					objResponse.put("success", memberId + " is added to " + gameId);
					Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_3, "" + memberId,
							true);
					Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_4, "" + gameId,
							true);
				}
	...
	/**
	 * Validate member and add to the database as the new member if he/she is not
	 * registered yet
	 * 
	...
	public Response memberLoginValidation() {
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"POST " + "gamification/games/validation", true);
		...
		if (name != "" && email != "") {// userData != null
			...
			Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_8, "" + member.getId(),
					true);
			return Response.status(HttpURLConnection.HTTP_OK).entity(objResponse.toJSONString())
					.type(MediaType.APPLICATION_JSON).build();
		} else {
	...
	// if time allows it, find a better implementation for this, where
	...
	public int isGameWithIdExist(String gameId) {
		...
		try {
			...
			if (gameAccess.isGameIdExist(conn, gameId)) {
				Context.getCurrent().monitorEvent(this, MonitoringEvent.RMI_SUCCESSFUL,
						"RMI isGameWithIdExist is invoked");
				return 1;
			} else {
				Context.getCurrent().monitorEvent(this, MonitoringEvent.RMI_SUCCESSFUL,
						"RMI isGameWithIdExist is invoked");
			...
			e.printStackTrace();
			Context.getCurrent().monitorEvent(this, MonitoringEvent.RMI_FAILED,
					"Exception when checking Game ID exists or not. " + e.getMessage());
			return 0;
		}
		// always close connections
		finally {
