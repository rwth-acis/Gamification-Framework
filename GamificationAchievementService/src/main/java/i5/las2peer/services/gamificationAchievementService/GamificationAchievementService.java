 ...
 * 
 * This Gamification Achievement Service is to manage achievement element in
...
@SwaggerDefinition(info = @Info(title = "Gamification Achievement Service", version = "0.1", description = "Gamification Achievement Service for Gamification Framework", termsOfService = "http://your-terms-of-service-url.com", contact = @Contact(name = "Muhammad Abduh Arifin", url = "dbis.rwth-aachen.de", email = "arifin@dbis.rwth-aachen.de"), license = @License(name = "your software license name", url = "http://your-software-license-url.com")))
@ManualDeployment
...
public class GamificationAchievementService extends RESTService {
	 ...
	 *
	 * @param gameId                   Game ID obtained from Gamification Game
	 *                                 Service
	 ...
	 * @param achievementbadgeid       The existing badge from Gamification Badge
	 *                                 Service - String (20 chars)
	 * @param achievementnotifcheckStr Achievement Notification Boolean - Boolean -
	 *                                 Option whether use notification or not. Must
	 *                                 be null for 'false' and any other value for
	 *                                 'true'
	...
	public Response createNewAchievement(
			...
			@ApiParam(value = "Achievement Notification Boolean - Boolean - Option whether use notification or not. NOTE: semantics are a little strange (because of backwards compatibility)! If the parameter is present, any value is considered as true. In order to set the value to value, you have to NOT send the parameter.") @FormDataParam("achievementnotificationcheck") String achievementnotifcheckStr,
		...
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"POST " + "gamification/achievements/" + gameId, true);
		...
		try {
			...
			try {
				...
				objResponse.put("message",
						"Cannot create achievement. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
			...
			if (achievementid != null) {
				...
				try {
					...
					Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_25, "" + gameId,
							true);
	 ...
	 * @return HTTP Response returned Achievement Model {@link AchievementModel} as
	 *         JSON object
	...
	public Response getAchievementWithId(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"GET " + "gamification/achievements/" + gameId + "/" + achievementId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			try {
				...
				try {
					...
					e1.printStackTrace();
					objResponse.put("message",
							"Cannot get achievement detail. Cannot check whether game ID exist or not. Database error. "
									+ e1.getMessage());
					Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
							(String) objResponse.get("message"));
					return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
	...
	public Response updateAchievement(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"PUT " + "gamification/achievements/" + gameId + "/" + achievementId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			try {
				...
				e1.printStackTrace();
				objResponse.put("message",
						"Cannot update achievement. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
				return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
	 ...
	 * @param gameId                   Game ID obtained from Gamification Game
	 *                                 Service
	 * @param achievementIds			Achievements to move to the back
	 *
	 * @return HTTP Response returned as JSON object
	 */
	@PUT
	@Path("/{gameId}/{achievementId}/up")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiResponses(value = {
			@ApiResponse(code = HttpURLConnection.HTTP_OK, message = "Achievement Updated"),
			@ApiResponse(code = HttpURLConnection.HTTP_INTERNAL_ERROR, message = "Error occured"),
	...
	@ApiOperation(value = "updateAchievement", notes = "A method to update an achievement with details (achievement ID, achievement name, achievement description, achievement point value, achievement point id, achievement badge id")
	public Response updateAchievementPosition(
			@ApiParam(value = "Game ID to update an achievement", required = true) @PathParam("gameId") String gameId,
			@ApiParam(value = "Achievement ID to be updated", required = true) @PathParam("achievementId") String achievementId,
			@ApiParam(value = "Achievement ID to be updated", required = true) @FormDataParam("achievementIds") String achievementIds) {
		/*
		 * TODO Consider breaking change and using default 'boolean' semantics (param
		 * needs to be string 'true' for true value)
		 */
		...
		try {
			...
			for(String s : split){
				AchievementModel ach = achievementAccess.getAchievementWithId(conn, gameId, s);
	...
	/**
	 * Legacy semantics of the 'achievementnotificationcheck' parameter are the
	 * following:
	 * - If parameter has any non-null value (even blank string) -> true
	 * - Otherwise -> false
	 *
	 * @param paramValue the raw string value of the form param (may be null if
	...
	public Response deleteAchievement(@PathParam("gameId") String gameId,
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"DELETE " + "gamification/achievements/" + gameId + "/" + achievementId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			try {
				...
				e1.printStackTrace();
				objResponse.put("message",
						"Cannot delete achievement. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
				return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
	...
	/**
	 * Get a list of achievements from database, support the features to do
	...
	public Response getAchievementList(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"GET " + "gamification/achievements/" + gameId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			try {
				...
				e1.printStackTrace();
				objResponse.put("message",
						"Cannot get achievements. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
				return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
