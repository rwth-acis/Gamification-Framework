...
//--------------------MAIN CONTENT PART--------------------//
		...
		var mainContentHandler = (function() {
			  ...
			  var target = $(e.target).attr("href") // activated tab
			  	switch(target) {
				    case "#pointtab":
				        ...
				        break;
				    case "#badgetab":
				    	$("#menucontent").load("badge.html", badgeModule.init);
				        break;
				    case "#achievementtab":
				    	$("#menucontent").load("achievement.html", achievementModule.init);
				        break;
				    case "#actiontab":
				    	$("#menucontent").load("action.html", actionModule.init);
				        break;
				    case "#questtab":
				    	$("#menucontent").load("quest.html", questModule.init);
				        break;
				     case "#leveltab":
				    	$("#menucontent").load("level.html", levelModule.init);
				        break;
				     case "#leaderboardtab":
			...
			console.dir($('#backtoappselection').data('events') );
		});

		function resetEventHandler(){
			$('#backtoappselection').unbind('click');
		}
		...
		function signinCallback(result) {
		    if(result === "success"){
		        ...
		        showLoginView(false);
		        console.log(window.localStorage["id_token"]);
