 ...
 * 
 * This is Gamification Action service to manage action as gamification element
 * in Gamification Framework
...
@Api(value = "/gamification/actions", authorizations = {
		@Authorization(value = "actions_auth", scopes = {
...
@SwaggerDefinition(info = @Info(title = "Gamification Action Service", version = "0.1", description = "Gamification Action Service for Gamification Framework", termsOfService = "http://your-terms-of-service-url.com", contact = @Contact(name = "Muhammad Abduh Arifin", url = "dbis.rwth-aachen.de", email = "arifin@dbis.rwth-aachen.de"), license = @License(name = "your software license name", url = "http://your-software-license-url.com")))
@ManualDeployment
...
public class GamificationActionService extends RESTService {
	 ...
	 * @param actionnotifcheckStr Action Notification Boolean - Boolean - Option
	 *                            whether use notification or not. Must be null for
	...
	public Response createNewAction(
		...
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"POST " + "gamification/actions/" + gameId, true);
		...
		try {
			...
			try {
				...
				objResponse.put("message",
						"Cannot create action. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
	 ...
	 * @return HTTP Response returned Action Model {@link ActionModel} as JSON
	 *         object
	 * @see ActionModel
	 */
	@GET
	...
	@ApiOperation(value = "Find action for specific Game ID and action ID", notes = "Returns a action", response = ActionModel.class, authorizations = @Authorization(value = "api_key"))
	public Response getActionWithId(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"GET " + "gamification/actions/" + gameId + "/" + actionId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			try {
				...
				try {
					...
					e1.printStackTrace();
					objResponse.put("message",
							"Cannot get action. Cannot check whether game ID exist or not. Database error. "
									+ e1.getMessage());
					Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
							(String) objResponse.get("message"));
					return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
				...
				if (action != null) {
					...
					Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_27, "" + gameId,
							true);
					return Response.status(HttpURLConnection.HTTP_OK).entity(actionString)
							.type(MediaType.APPLICATION_JSON).build();
				} else {
	...
	public Response updateAction(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"PUT " + "gamification/actions/" + gameId + "/" + actionId, true);
		long randomLong = new Random().nextLong(); // To be able to match
	...
	public Response deleteAction(@PathParam("gameId") String gameId,
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"DELETE " + "gamification/actions/" + gameId + "/" + actionId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			try {
				...
				e1.printStackTrace();
				objResponse.put("message",
						"Cannot delete action. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
				return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
	...
	/**
	 * Get a list of actions from database, support the features to do pagination
	 * and search
	 * 
	 * @param gameId       Game ID obtained from Gamification Game Service
	...
	public Response getActionList(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"GET " + "gamification/actions/" + gameId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			try {
				...
				e1.printStackTrace();
				objResponse.put("message",
						"Cannot get actions. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
				return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
	...
	/**
				...
				public String triggerActionRMI(String gameId, String memberId, String actionId)  {
					...
					try {
						...
						// add check whether is lrs type, whether is distinct, whether already there
	...
	// i could use this instead of doing the mini client thing?
	/**
