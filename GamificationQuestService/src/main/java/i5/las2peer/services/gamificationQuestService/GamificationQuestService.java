 ...
 * 
 * This is Gamification Quest Service to manage quest element in Gamification
 * Framework
...
@Api(value = "/quests", authorizations = {
		@Authorization(value = "quests_auth", scopes = {
...
@SwaggerDefinition(info = @Info(title = "Gamification Quest Service", version = "0.1", description = "Gamification Quest Service for Gamification Framework", termsOfService = "http://your-terms-of-service-url.com", contact = @Contact(name = "Muhammad Abduh Arifin", url = "dbis.rwth-aachen.de", email = "arifin@dbis.rwth-aachen.de"), license = @License(name = "your software license name", url = "http://your-software-license-url.com")))
@ManualDeployment
...
public class GamificationQuestService extends RESTService {
	...
	/**
	 * Get an element of JSON object with specified key as list of pair string and
	 * integer
	 * 
	 * @return list of pair string and integer value
	 * @throws IOException IO exception
	 */
	private static List<Pair<String, Integer>> listPairfromJSON(JSONObject obj, String mainkey, String keykey,
	 ...
	 * <li>questachievementid - Achievement name obtained Gamification Achievement
	 ...
	 * <li>questidcompleted - Completed Quest ID if the quest flag dependency is
	 * true - String (20 chars)
	 * <li>questpointvalue - Point value if the point flag dependency boolean is
	 * true - Integer
	 * <li>questactionids - Array of object {action: , times: }(times must have a
	 * positive value)
	 * <li>questdescription - Quest Description - String (50 chars)
	 * <li>questnotificationcheck - Quest Notification Boolean - Boolean - Option
	 * whether use notification or not
	...
	public Response createNewQuest(
		...
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"POST " + "gamification/quests/" + gameId, true);
		...
		try {
			...
			try {
				...
				objResponse.put("message",
						"Cannot create quest. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
				return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
			...
			e.printStackTrace();
			objResponse.put("message",
					"Cannot create quest. NullPointerException. Failed to upload " + questid + ". " + e.getMessage());
			Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR, (String) objResponse.get("message"));
			return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
					.type(MediaType.APPLICATION_JSON).build();
	...
	public Response getQuestWithId(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"GET " + "gamification/quests/" + gameId + "/" + questId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			try {
				...
				e1.printStackTrace();
				objResponse.put("message",
						"Cannot get quest. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
				return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
	...
	public Response updateQuest(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"PUT " + "gamification/quests/" + gameId + "/" + questId, true);
		long randomLong = new Random().nextLong(); // To be able to match
...
/**
	 ...
	 *                                 Service
	 * @param questId            Achievement ID to be updated
	 * @param gameId                   Game ID obtained from Gamification Game
	 *                                 Service
	 * @param questIds			Achievements to move to the back
	 *
	 * @return HTTP Response returned as JSON object
	 */
	@PUT
	@Path("/{gameId}/{questId}/up")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiResponses(value = {
	...
	public Response updateQuestPosition(
			@ApiParam(value = "Game ID to update an achievement", required = true) @PathParam("gameId") String gameId,
			@ApiParam(value = "Achievement ID to be updated", required = true) @PathParam("questId") String questId,
			@ApiParam(value = "Achievement ID to be updated", required = true) @FormDataParam("questIds") String questIds) {
		/*
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"PUT " + "gamification/achievements/" + gameId + "/" + questId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			QuestModel currentQuest = questAccess.getQuestWithId(conn, gameId, questId);
			
			ArrayList<QuestModel> quests = new ArrayList<QuestModel>();
			quests.add(currentQuest);
			for(String s : split){
				QuestModel quest = questAccess.getQuestWithId(conn, gameId, s);
				quests.add(quest);
			}


			questAccess.moveUp(conn, gameId, quests);
	...
	public Response deleteQuest(@PathParam("gameId") String gameId,
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"DELETE" + "gamification/quests/" + gameId + "/" + questId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			try {
				...
				e1.printStackTrace();
				objResponse.put("message",
						"Cannot delete quest. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
				return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
	...
	public Response getQuestList(
		...
		// Request log
		Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_99,
				"GET " + "gamification/quests/" + gameId, true);
		long randomLong = new Random().nextLong(); // To be able to match
		...
		try {
			...
			try {
				...
				e1.printStackTrace();
				objResponse.put("message",
						"Cannot get quests. Cannot check whether game ID exist or not. Database error. "
								+ e1.getMessage());
				Context.getCurrent().monitorEvent(this, MonitoringEvent.SERVICE_ERROR,
						(String) objResponse.get("message"));
				return Response.status(HttpURLConnection.HTTP_INTERNAL_ERROR).entity(objResponse.toJSONString())
